// ============================================================
// AutoTrader Bot — Railway Server v4.0
// - E*Trade OAuth flow at /etrade/auth
// - Auto token renewal every 90 min during market hours
// - OneSignal push notifications when token expires
// - Premarket screener at 9 AM ET
// ============================================================
...
// Sends push to ALL subscribed devices for this app
// For multi-user: filter by external_id (user's email/id)
async function sendPush(cfg, title, message, url = '') {
  if (!cfg.onesignal_app_id || !cfg.onesignal_api_key) {
  ...
  try {
    ...
    if (j.errors) {
      console.log('[PUSH] Error:', JSON.stringify(j.errors));
    } else {
      console.log(`[PUSH] Sent to ${j.recipients} devices`);
    }
  } catch (e) {
...
// ─── E*TRADE TOKEN RENEWAL ───────────────────────────────────
let lastRenewal    = null;
let renewalWarned  = false;
const RENEW_MS     = 90 * 60 * 1000; // renew every 90 minutes
const WARN_MS      = 30 * 60 * 1000; // re-notify every 30 min if failed
...
async function renewToken(cfg) {
  ...
  try {
    const r    = await fetch(renewUrl, { headers: { Authorization: makeOAuthHeader('GET', renewUrl, cfg) } });
    const text = await r.text();
    if (r.ok && text.includes('Access Token has been renewed')) {
      lastRenewal   = new Date();
      renewalWarned = false;
      await addLog('info', 'E*Trade token renewed successfully');
      return true;
    }
    await addLog('err', `Token renewal failed: ${text}`);
    return false;
  } catch (e) {
    await addLog('err', `Token renewal error: ${e.message}`);
    return false;
  }
}
...
async function checkAndRenewToken() {
  ...
  if (!cfg?.e_token || cfg?.sandbox) return; // skip if no token or sandbox mode
  const now = new Date();

  // Renew if it's been 90+ minutes since last renewal
  if (!lastRenewal || (now - lastRenewal) >= RENEW_MS) {
    await addLog('info', 'Attempting E*Trade token renewal...');
    const ok = await renewToken(cfg);
    if (!ok) {
      // Renewal failed — token may be fully expired, need new PIN
      if (!renewalWarned || (now - renewalWarned) >= WARN_MS) {
        renewalWarned = now;
        const authUrl = `${process.env.RAILWAY_PUBLIC_DOMAIN ? 'https://' + process.env.RAILWAY_PUBLIC_DOMAIN : ''}/etrade/auth`;
        await addLog('warn', 'E*Trade token expired — push notification sent to re-authorize');
        await sendPush(cfg,
          'AutoTrader — Action Required',
          'Your E*Trade session has expired. Tap to re-authorize and resume trading.',
          authUrl
        );
        // Pause bot until re-authorized
...
async function placeOrder(sym, action, shares, price, cfg) {
  ...
  try {
    ...
    if (r.status === 401) {
      await addLog('err', 'E*Trade 401 during order — token expired');
      const authUrl = `${process.env.RAILWAY_PUBLIC_DOMAIN ? 'https://' + process.env.RAILWAY_PUBLIC_DOMAIN : ''}/etrade/auth`;
      await sendPush(cfg, 'AutoTrader — Trade Failed', `Could not place ${action} order for ${sym} — session expired. Tap to re-authorize.`, authUrl);
      return { success: false, expired: true };
    }
    const orderId = j?.PlaceOrderResponse?.OrderIds?.[0]?.orderId;
    if (orderId) {
      await addLog('buy', `E*Trade order confirmed — orderId: ${orderId}`);
      await sendPush(cfg, `AutoTrader — ${action} Executed`, `${action} ${shares}sh ${sym} @ $${price.toFixed(2)} | Target: $${(price * 1.012).toFixed(2)}`);
      return { success: true, orderId };
    }
    await addLog('err', `E*Trade order failed: ${JSON.stringify(j)}`);
    return { success: false };
  } catch (e) {
    await addLog('err', `E*Trade error: ${e.message}`);
    return { success: false };
  }
}
...
app.get('/etrade/auth', async (req, res) => {
  ...
  if (!cfg?.e_key || !cfg?.e_secret) {
    return res.send(`<html><body style="font-family:monospace;background:#050a0f;color:#b8d4ee;padding:30px">
      ...
      <p>Add your Consumer Key and Consumer Secret in your phone app Config first.</p>
    </body></html>`);
  }
  ...
  try {
    ...
    const r      = await fetch(reqUrl, { headers: { Authorization: header } });
    const text   = await r.text();
    const p      = parseQS(text);
    if (!p.oauth_token) return res.send(`<html><body style="font-family:monospace;background:#050a0f;color:#f0364a;padding:30px">
      <h2>Error getting request token</h2><pre>${text}</pre>
      <p>Make sure your Consumer Key and Secret are correct in Config.</p>
    </body></html>`);
    ...
    res.send(`<!DOCTYPE html><html><head>
      ...
      <style>
        ...
        input{width:100%;background:#0d1c30;border:1px solid #1c3a59;border-radius:4px;padding:12px;color:#b8d4ee;font-family:monospace;font-size:18px;box-sizing:border-box;margin:8px 0;letter-spacing:.1em}
      ...
      <div class="box">
        ...
        <p>Tap the button below to open E*Trade authorization. Log in and approve the bot. You will see a <strong style="color:#00e891">5-digit PIN</strong> — copy it.</p>
        <a class="btn-link" href="${authUrl}" target="_blank">Tap to Authorize on E*Trade →</a>
        <p class="note">After you see the PIN, come back here and enter it below. Do not close this page.</p>
      </div>
      <div class="box">
        <h3>Step 2 — Enter your PIN</h3>
        <input type="text" id="pin" placeholder="XXXXX" inputmode="numeric" maxlength="10" autocomplete="off"/>
        <button onclick="go()">Complete Authorization</button>
        <p class="note">Tokens are saved permanently to your database. Bot resumes trading automatically.</p>
      </div>
      <div id="msg"></div>
      <script>
        async function go() {
          ...
          try {
            const r = await fetch('/etrade/callback?rt=${p.oauth_token}&pin=' + encodeURIComponent(pin));
            const j = await r.json();
            const el = document.getElementById('msg');
            if (j.ok) {
              el.style.cssText = 'display:block;background:#0a2a1a;border:1px solid #00e891;border-radius:5px;padding:16px';
              el.innerHTML = '<p style="color:#00e891;font-size:16px;margin:0">Authorization complete! Trading resumed. You can close this page.</p>';
            } else {
              el.style.cssText = 'display:block;background:#2a0a0a;border:1px solid #f0364a;border-radius:5px;padding:16px';
              el.innerHTML = '<p style="color:#f0364a;margin:0">Error: ' + j.error + '<br><br>Try tapping the E*Trade button again and get a fresh PIN.</p>';
            }
          } catch(e) {
...
app.get('/etrade/callback', async (req, res) => {
  ...
  try {
    ...
    const header = makeOAuthHeader('GET', accUrl, tmpCfg, { oauth_verifier: pin });
    const r      = await fetch(accUrl, { headers: { Authorization: header } });
    const text   = await r.text();
    const p      = parseQS(text);
    if (!p.oauth_token) return res.json({ ok: false, error: 'Invalid PIN or expired request token. Try authorizing again.' });
    await patchConfig({ e_token: p.oauth_token, e_token_secret: p.oauth_token_secret });
    lastRenewal   = new Date();
    renewalWarned = false;
    await addLog('info', 'E*Trade OAuth complete — tokens saved, bot resuming');
    await patchState({ status_text: 'E*Trade authorized — bot resuming' });
    // Send confirmation push
    const freshCfg = await getConfig();
    await sendPush(freshCfg, 'AutoTrader — Authorized', 'E*Trade connected. Bot is now trading your account.');
    res.json({ ok: true });
...
function macd(c, f = 12, s = 26, sg = 9) {
  ...
  for (let i = 0; i < se.length; i++) { const x = off + i; if (se[i] != null) { sl[x] = se[i]; hl[x] = ml[x] - se[i]; } }
  return { ml, sl, hl };
}
...
function calcSlope(c, p = 20) {
  ...
  const s = c.slice(-p), n = s.length, sx = n * (n - 1) / 2, sx2 = n * (n - 1) * (2 * n - 1) / 6;
...
async function fetchCandles(sym, interval, lookback, tdKey) {
  const r = await fetch(`${TD}/time_series?symbol=${sym}&interval=${interval}&outputsize=${lookback}&order=ASC&apikey=${tdKey}`);
  const j = await r.json();
...
Return ONLY: {"grade":"B","score":72,"action":"BUY","confidence":70,"reason":"one sentence","target":${(ind.px * 1.012).toFixed(2)},"stop":${(ind.px * 0.988).toFixed(2)},"strongest":"MACD","avoid":false}`;
  try {
...
function updateCoefs(coefs, indSnap, result) {
  ...
  Object.keys(out).forEach(k => {
    ...
    out[k] = clamp(out[k] + (s.bull === null ? 0 : correct ? 0.1 : -0.06));
  });
  return out;
}
...
async function monitorPosition(state, cfg) {
  ...
  if (pnl >= 1.0 || px <= t.stop_price || days >= 3) {
    ...
    // Push notification on close
    await sendPush(cfg,
      `AutoTrader — ${result} ${pnl >= 0 ? '+' : ''}${pnl.toFixed(2)}%`,
      `${t.symbol} closed: ${why} | ${pnl >= 0 ? '+' : ''}${pnl.toFixed(2)}% | New balance: $${(state.balance * (1 + pnl / 100)).toFixed(2)}`
    );
  } else {
...
async function scanCycle() {
  ...
  try {
    ...
    // Check token renewal during market hours
